...
6. Render transport controls (play/stop, tempo +/- with a number input), concise track rows (mute, volume slider, deafen, beats input, trash, plus an add button), and a header with a theme toggle and commit hash.
7. Style with CSS variables so light/dark themes are easy to flip, keep buttons neutral, center the +/- glyphs, and make inputs compact.
